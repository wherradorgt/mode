...
def get(df,columns):
    """
    Obtiene la mediana de una o varias columnas de un dataset si estan son columnas numericas no categoricas
...
def operand(column_list):
    ...
    Returns:
        (int | float): devuelme la media de la lista de la columna y si es categorica devuelve None
    """
    if pd.api.types.is_categorical_dtype(column_list) or pd.api.types.is_string_dtype(column_list):
        return None
    else:
       return np.median(column_list)
